<#
.SYNOPSIS
    Stages the Azure file share listing (tab-delimited text) into SQL Server,
    ...
    Workflow:
      ...
      2. For each .txt file in SourceFolder:
           ...
           b. Compare those to the values recorded the last time this
              SourceFile was loaded (dbo.fileshare_data_load_history).
...
param(
    ...
    [string]$TargetTable = "dbo.fileshare_data",
    [string]$StagingTable = "dbo.fileshare_data_staging",
    [string]$HistoryTable = "dbo.fileshare_data_load_history",

    [string]$SourceFolder = "\\AZUKSWVPUNSD01\Temp\Unstructured\FileListing\FileShare",
