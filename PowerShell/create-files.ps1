...
function Create-ExcelFile2003 {
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $excel.Visible = $false
        $workbook = $excel.Workbooks.Add()
        $workbook.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",1)
        $workbook.Close($true)
        $excel.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null
        $excel = $null
...
function Create-WordFile2003 {
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $word.Visible = $false
        $document = $word.Documents.Add()
        $document.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",1)
        $document.Close()
        $word.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($word) | Out-Null
        $word = $null
...
function Create-ExcelFilePwd {
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $excel.Visible = $false
        $workbook = $excel.Workbooks.Add()
        $workbook.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",51,"Test")
        $workbook.Close($true)
        $excel.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null
        $excel = $null
...
function Create-WordFilePwd {
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $word.Visible = $false
        $document = $word.Documents.Add()
        $document.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",19,$false,"Test")
        $document.Close()
        $word.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($word) | Out-Null
        $word = $null
...
# Function to create Excel files
function Create-ExcelFile2003Pwd {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFile2003Pwd"
    $strExtension = ".xls"
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $excel.Visible = $false
        $workbook = $excel.Workbooks.Add()
        $workbook.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",56,"Test")
        $workbook.Close($true)
        $excel.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null
        $excel = $null
...
# Function to create Word files
function Create-WordFile2003Pwd {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFile2003Pwd"
    $strExtension = ".doc"
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $word.Visible = $false
        $document = $word.Documents.Add()
        $document.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",0,$false,"Test")
        $document.Close()
        $word.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($word) | Out-Null
        $word = $null
...
# Function to create Excel files
function Create-ExcelFilePwdM {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFilePwdM"
    $strExtension = ".xlsx"
    $strNewPath = "$strPath\Excel"
    if (!(Test-Path $strNewPath)) {
        New-Item -ItemType Directory -Path $strNewPath
...
# Function to create PowerPoint files
function Create-PowerPointFilePwdM {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFilePwdM"
    $strExtension = ".pptx"
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $presentation = $powerPoint.Presentations.Add(0)
        $presentation.SaveAs("$strNewPath\$strFilename$intCounter$strExtension")
	$presentation.WritePassword = "Test"
        $presentation.Close()
        $powerPoint.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($powerPoint) | Out-Null
        $powerPoint = $null
...
# Function to create Excel files
function Create-ExcelFile2003PwdM {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFile2003PwdM"
    $strExtension = ".xls"
    $strNewPath = "$strPath\Excel"
    if (!(Test-Path $strNewPath)) {
        New-Item -ItemType Directory -Path $strNewPath
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $excel.Visible = $false
        $workbook = $excel.Workbooks.Add()
        $workbook.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",56)
	$workbook.WritePassword = "Test"
        $workbook.Close($true)
        $excel.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($excel) | Out-Null
...
# Function to create Word files
function Create-WordFile2003PwdM {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFile2003PwdM"
    $strExtension = ".doc"
    ...
    for ($intCounter = 1; $intCounter -le $countFiles; $intCounter++) {
        ...
        $word.Visible = $false
        $document = $word.Documents.Add()
        $document.SaveAs("$strNewPath\$strFilename$intCounter$strExtension",0)
	$document.WritePassword = "Test"
        $document.Close()
        $word.Quit()
        [System.Runtime.InteropServices.Marshal]::ReleaseComObject($word) | Out-Null
...
# Function to create PowerPoint files
function Create-PowerPointFile2003PwdM {
    param ([string]$strPath,
            [Int]$countFiles)
    $strFilename = "TestFile2003PwdM"
    $strExtension = ".ppt"
...
Create-ExcelFile2003 -strPath $targetDir -countFiles 2
Create-WordFile2003 -strPath $targetDir -countFiles 2
Create-ExcelFilePwd -strPath $targetDir -countFiles 2
Create-WordFilePwd -strPath $targetDir -countFiles 2
Create-ExcelFile2003Pwd -strPath $targetDir -countFiles 2
Create-WordFile2003Pwd -strPath $targetDir -countFiles 2
Create-ExcelFilePwdM -strPath $targetDir -countFiles 2
Create-WordFilePwdM -strPath $targetDir -countFiles 2
Create-PowerPointFilePwdM -strPath $targetDir -countFiles 2
Create-ExcelFile2003PwdM -strPath $targetDir -countFiles 2
Create-WordFile2003PwdM -strPath $targetDir -countFiles 2
Create-PowerPointFile2003PwdM -strPath $targetDir -countFiles 2
